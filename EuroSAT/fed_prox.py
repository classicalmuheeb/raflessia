...
def main() -> None:
    ...
    parser.add_argument('--iid', type=bool, default=True, help='Specify if dataset should be iid')
    parser.add_argument('--balance', type=bool, default=False, help='Specify if dataset should be balanced')
    parser.add_argument('--strategy', type=str, default='FedProx', help='Specify strategy')
    ...
    if DEVICE.type == "cuda":
        client_resources["num_gpus"] = 1.0/2 
    ...
    FedProxStrategy = fl.server.strategy.FedProx(
        fraction_fit=0.1, #the fraction of available clients to use for evaluation of the model at each round of Federated Learning
        min_fit_clients= 0.1, #the minimum number of clients that should be selected for training at each round of Federated learning
